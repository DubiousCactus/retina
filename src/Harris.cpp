...
HarrisExtractor::Extract(const Frame* frame)
{
    ...
    std::cout << "[*] " << this->keypoints.size() << " keypoints found!"
              << std::endl;
    return this->keypoints;
}

void
HarrisExtractor::NonMaxSuppression(int window_size,
                                   Matrix<double>& harrisResponse)
{
    int offset = window_size / 2;
    for (int i = window_size; i < harrisResponse.Rows() - window_size; ++i) {
        for (int j = window_size; j < harrisResponse.Cols() - window_size;
             ++j) {
...
HarrisExtractor::MeasureCorner(const Frame* f, int x, int y, int block_size)
{
    ...
    Sxx = Matrix<double>::Sum(
      f_xx, f_xx.Rows() / 2, f_xx.Cols() / 2, this->window_size);
    Syy = Matrix<double>::Sum(
      f_yy, f_yy.Rows() / 2, f_yy.Cols() / 2, this->window_size);
    Sxy = Matrix<double>::Sum(
      f_xy, f_xy.Rows() / 2, f_xy.Cols() / 2, this->window_size);
    det = (Sxx * Syy) - pow(Sxy, 2);
    trace = Sxx + Syy;
